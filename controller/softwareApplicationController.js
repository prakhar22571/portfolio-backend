...
export const addNewApplication = catchAsyncErrors(async (req, res, next) => {
  if (!req.files || Object.keys(req.files).length === 0) {
    return next(
      new ErrorHandler("Developer Tool Icon/Image Required!", 404)
    );
  }
  const { svg } = req.files;
  const { name } = req.body;
  ...
  const cloudinaryResponse = await cloudinary.uploader.upload(
    svg.tempFilePath,
    { folder: "PORTFOLIO DEVELOPER TOOL IMAGES" }
  );
  ...
  res.status(201).json({
    success: true,
    message: "New Developer Tool Added!",
    softwareApplication,
  });
});
...
export const deleteApplication = catchAsyncErrors(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    message: "Developer Tool Deleted!",
  });
});
